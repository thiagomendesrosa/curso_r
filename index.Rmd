...
A Codeplan conta com o RStudio server, o que significa que você pode acessar e trabalhar de qualquer máquina conectada à rede do GDF diretamente do navegador. Clique [aqui](http://10.233.35.55:8787/) para acessar o servidor.

Uma vantagem de utilizar o Rserver é sua alta capacidade de processamento, quando comparado com as máquinas locais, contando atualmente com 32GB de memória RAM e um processador de 16 núcleos.

Além disso, os pacotes instalados para a realização de um projeto ficam disponíveis para os demais usuários da empresa, sendo periodicamente atualizados pelo administrador do servidor. Isso facilita a reprodutibilidade dos códigos escritos por outros usuários, auxiliando também na continuidade de pesquisas e projetos. Sempre que precisar instalar um pacote, comunique o administrador do servidor [Luiz.rubens@codeplan.df.gov.br ou thiago.rosa@codeplan.df.gov.br] para que ele também fique disponível para os demais usuários. Caso você necessite um pacote que demande alguma atualização do servidor, contate o administrador do servidor explicando os detalhes.
...
O primeiro passo é efetuarmos o login no RStudio server da Codeplan. Para isso, acesse o site do [servidor](http://10.233.35.55:8787/).
...
Agora vamos realizar algumas operações matemáticas básicas com o R. Digite diretamente no console ou abra um script, escreva os códigos e utilize o atalho de teclado para rodar o código (você pode também rodar o código com o botão "Run" na parte superior da janela de script).
...
Além disso, no RStudio, você conta com as *cheatsheets*, na barra superior de ajuda (`Help`). Você poderá baixar diversos pdfs com "colas" para as principais operações realizadas na análise de dados.
...
Como boa prática, é costume que apenas os códigos sejam carregados no `Github`. Para ignorar a pasta de dados, abra o arquivo `.gitignore` (caso que não tenha sido criado junto com o projeto, pasta criar um arquivo de texto com esse nome) e adicione uma linha com a informação `/dados`.
...
info <- getURL("http://www.codeplan.df.gov.br/microdados-pdad-2018/",
               verbose=TRUE,
               ftp.use.epsv=TRUE, 
               dirlistonly = TRUE)
...
pdf <- info[str_detect(info,"2018.pdf|Impressao.pdf")] %>%
  # Pegar a URL
  str_match('href=(.*?).pdf') %>% 
  # Tratar a URL
  str_remove('href="') %>% 
...
```{r}
pdad_2018_moradores <- read.csv2("dados/mor2018_31ras.csv", stringsAsFactors=FALSE)
```
...
```{r, eval=FALSE}
library(readr)
pdad_2018_moradores <- read_delim("http://www.codeplan.df.gov.br/wp-content/uploads/2020/06/mor2018_31ras.csv", 
...
```{r}
# Consultar colunas
colunas <- RODBC::sqlColumns(db,"pdad.mor2018_31ras")
```
...
RODBC::sqlColumns(db,"pdad.mor2018_31ras") %>%
...
Vamos, então, carregar a base de pessoas. A estrutura para realizar uma consulta é bastante simples: dizemos a ação que queremos realizar que, no caso da consulta, é feita com a palavra `select`; informamos as variáveis a serem carregadas, separando-as por vírgulas; dizemos de onde queremos obter esses dados, com a palavra `from`; e informamos o local em que a tabela está armazenada no banco de dados, na estrutura "esquema.tabela", no nosso caso, `pdad.mor2018_31ras`.
...
pdad_2018_moradores <- RODBC::sqlQuery(db,"select A01ra from pdad.mor2018_31ras")
```
...
```{r}
pdad_2018_moradores <- DBI::dbGetQuery(db,"select A01ra from pdad.mor2018_31ras")
```
...
pdad_2018_moradores <- DBI::dbGetQuery(db,"select * from pdad.mor2018_31ras")
```
...
pdad_2018_domicilios <- DBI::dbGetQuery(db,"select * from pdad.dom2018_31ras")
```
...
# Criar os pontos para inserir os labels
# Ajustar objeto
pontos <- fortify(RA) %>%
  # Agrupar por RA
  dplyr::group_by(id) %>%
  # Pegar a latitude e longitude médias
  dplyr::summarise(lat=mean(lat),
                   long=mean(long)) %>%
  # Deixar o id como integer (para o join)
...
pdad_mor<- DBI::dbGetQuery(db,"select A01ra, FATOR_PROJ from pdad.mor2018_31ras") %>%
  # Agrupar por RA
...
Agora, temos um inflator para atualizar os valores monetários da PDAD 2018 para o mês mais recente (dezembro de 2019). Iremos utilizar essa informação mais tarde, para construirmos a renda em valores atualizados.

Vamos aproveitar a base do IPCA para aprendermos uma manipulação muito importante de bases de dados. Para fazer gráficos, o formato `long` é muito útil, enquanto o formato `wide` apresenta a tabela no formato de colunas. Vamos treinar estes conceitos carregando a inflação, de 2012 até 2019, do DF, do Brasil, de São Paulo e do Rio de Janeiro.
...
Por essa metodologia, a renda domiciliar nominal estimada foi de R\$ `r rnd[[1,"renda_dom"]]`, com intervalo de R\$ `r rnd[[1,"renda_dom_low"]]` e R\$ `r rnd[[1,"renda_dom_upp"]]`. Em termos reais (Dezembro/2019), o valor estimado é de R\$ `r rnd[[1,"renda_dom_real"]]`, com intervalo de R\$ `r rnd[[1,"renda_dom_real_low"]]` a R\$ `r rnd[[1,"renda_dom_real_upp"]]`.
...
Comumente, utilizamos o primeiro *chunk* para inserir as configurações e opções a serem utilizadas no decorrer do documento. Note que utilizamos a opção `include=FALSE` para que este *chunk* não seja incluído no documento. Nele, carregamos os pacotes necessários, ajustamos as configurações padrão dos demais *chunks*, carregamos a base e ajustamos uma configuração do pacote `kanleExtra`, que será utilizado para gerar nossas tabelas no formato $\LaTeX$. 
...
# ## Salários
# 
# No que diz respeito aos salários, sua distribuição por faixas de lário mínimo^[O salário considerado foi de R$ 954,00] é apresentada  Figura \ref{fig:salarios}.
# 
