...
Esse formato, em que temos cada coluna um caso, é o chamado formato `wide`. Caso a necessidade fosse fazer um gráfico da evolução da inflação em cada localidade, seria necessário construir uma geometria para cada caso. Vamos transformar esses dados para o formato `long`, em qua cada linha será um caso, utilizando o pacote `tidyr`.
...
A função `gather()` "junta" os valores para colocar cada caso em uma linha. Vamos fazer agora o caminho inverso, voltando para o formato `wide` com a função `spread`.
...
O primeiro argumento é a variável chave, que irá assumir o papel de coluna, enquanto o segundo argumento é a variável dos valores, que irá popular estas colunas. Vamos contruir o gráfico da inflação destas localidades, utilizando o formato `long`.
